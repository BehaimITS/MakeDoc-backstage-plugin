...
const STANDARD_AWS_HOST_RE = /^(?:(?<bucket>[a-z0-9.-]+)\.)?s3(?:[.-](?<region>[a-z0-9-]+))?\.amazonaws\.com(?:\.cn)?$/;
const VPC_ENDPOINT_HOST_RE = /^(?<bucket>[a-z0-9.-]+)\.bucket\.(?<vpceId>vpce-[a-z0-9-]+)\.s3[.-](?<region>[a-z0-9-]+)\.vpce\.amazonaws\.com(?:\.cn)?$/;
...
function isAmazonHost(host) {
  return host === "amazonaws.com" || host === "amazonaws.com.cn" || host.endsWith(".amazonaws.com") || host.endsWith(".amazonaws.com.cn");
}
function parseAmazonUrl(url, host, pathname, config) {
