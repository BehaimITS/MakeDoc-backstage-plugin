...
class CacheManager {
  ...
  static fromConfig(config$1, options = {}) {
    ...
    const connection = CacheManager.parseConnection(config$1);
   ...
   */
  static parseStoreOptions(config, deps) {
   ...
   */
  static parseRedisOptions(config$1, deps) {
    const redisOptions = {
      type: "redis"
    };
    const redisConfig = deps.config.getOptionalConfig(config$1.storeConfigPath) ?? new config.ConfigReader({});
    redisOptions.client = {
    ...
    if (redisConfig.has("cluster")) {
      ...
      let defaults = clusterConfig.getOptional("defaults");
      if (typeof config$1.connection === "object") {
        const { url: _url, ...connectionDefaults } = config$1.connection;
   ...
   */
  static parseValkeyOptions(config$1, deps) {
    const valkeyOptions = {
      type: "valkey"
    };
    const valkeyConfig = deps.config.getOptionalConfig(config$1.storeConfigPath) ?? new config.ConfigReader({});
  ...
  /** @internal */
  constructor(store, connection, errorHandler, logger, defaultTtl, storeOptions) {
